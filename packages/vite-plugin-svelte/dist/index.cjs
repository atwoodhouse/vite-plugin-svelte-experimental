...
function getSvelteDependencies(deps, pkgDir, path10 = []) {
  const result = [];
  ...
  for (const { pkg, dir } of resolvedDeps) {
    ...
    if (!type)
      continue;
    result.push({ name: pkg.name, type, pkg, dir, path: path10 });
    if (type === "component-library" && pkg.dependencies) {
      let dependencyNames = Object.keys(pkg.dependencies);
      const circular = dependencyNames.filter((name) => path10.includes(name));
      if (circular.length > 0) {
        log.warn.enabled && log.warn(
          `skipping circular svelte dependencies in automated vite optimizeDeps handling`,
          circular.map((x) => path10.concat(x).join(">"))
        );
        dependencyNames = dependencyNames.filter((name) => !path10.includes(name));
      }
      if (path10.length === 3) {
        log.debug.once(`encountered deep svelte dependency tree: ${path10.join(">")}`);
      }
      result.push(...getSvelteDependencies(dependencyNames, dir, path10.concat(pkg.name)));
    }
  }
  return result;
}
...
function idToFile(id) {
  if (id.startsWith(FS_PREFIX)) {
    id = id.slice(IS_WINDOWS2 ? FS_PREFIX.length : FS_PREFIX.length - 1);
  }
...
// src/ui/inline-editor/plugin.ts
var import_vite5 = require("vite");
var import_path9 = __toESM(require("path"), 1);
var import_url3 = require("url");
var import_fs7 = __toESM(require("fs"), 1);
var defaultInspectorOptions2 = {
...
function svelteInlineEditor() {
  ...
  return {
    ...
    configResolved(config) {
      var _a, _b, _c;
      const vps = config.plugins.find((p) => p.name === "vite-plugin-svelte");
      if (!((_c = (_b = (_a = vps == null ? void 0 : vps.api) == null ? void 0 : _a.options) == null ? void 0 : _b.experimental) == null ? void 0 : _c.inlineEditor)) {
    ...
    configureServer(server) {
      ...
      server.ws.on("svelte-inline-editor:start", (meta, client) => {
        try {
          const filePath = import_path9.default.resolve(root, meta.loc.file);
      ...
      server.ws.on("svelte-inline-editor:save", (data) => {
        const { compileSvelte: compileSvelte2, requestParser, options } = api;
        const { code, content, file } = data;
        try {
          const filePath = import_path9.default.resolve(root, file);
          const diskContent = import_fs7.default.readFileSync(filePath, "utf-8");
          if (content === diskContent) {
            compileSvelte2(requestParser(file, false), code, options).then(() => {
              import_fs7.default.writeFileSync(file, code, "utf-8");
    ...
    async load(id, options) {
      ...
      if (id === "virtual:svelte-inline-editor-options") {
        ...
        if (import_fs7.default.existsSync(file)) {
          return await import_fs7.default.promises.readFile(file, "utf-8");
        } else {
          log.error(`failed to find file for svelte-inspector: ${file}, referenced by id ${id}.`);
        }
      }
  ...
  const plugins = [
    {
      ...
      async configResolved(config) {
        ...
        api.compileSvelte = compileSvelte2;
        api.cache = cache;
