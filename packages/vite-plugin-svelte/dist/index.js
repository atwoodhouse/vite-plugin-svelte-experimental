...
function getInlineEditorPath() {
  const pluginPath = normalizePath4(path9.dirname(fileURLToPath2(import.meta.url)));
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.ws.on("svelte-inline-editor:start", (meta, client) => {
        try {
          const filePath = path9.resolve(root, meta.loc.file);
      ...
      server.ws.on("svelte-inline-editor:save", (data) => {
        ...
        try {
          const filePath = path9.resolve(root, file);
          const diskContent = fs8.readFileSync(filePath, "utf-8");
          if (content === diskContent) {
            compileSvelte2(requestParser(file, false), code, options).then(() => {
              fs8.writeFileSync(file, code, "utf-8");
    ...
    async load(id, options) {
      ...
      if (id === "virtual:svelte-inline-editor-options") {
        ...
        if (fs8.existsSync(file)) {
          return await fs8.promises.readFile(file, "utf-8");
        } else {
          log.error(`failed to find file for svelte-inspector: ${file}, referenced by id ${id}.`);
        }
      }
      ...
      load(id, opts) {
        ...
        if (svelteRequest) {
          ...
          if (viteConfig.assetsInclude(filename)) {
            ...
            return fs9.readFileSync(filename, "utf-8");
          }
        }
      },
